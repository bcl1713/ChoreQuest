...
describe("FantasyButton", () => {
  const mockUseReducedMotion =
    useReducedMotionModule.useReducedMotion as jest.MockedFunction<
  ...
  describe("Rendering", () => {
    ...
    it("should render inline icon children on the same line via inline-flex wrapper", () => {
      render(
        <FantasyButton>
      ...
      const childrenSpan = button.querySelector(":scope > span:last-child");
      expect(childrenSpan).toHaveClass("inline-flex", "items-center");
      expect(screen.getByTestId("inline-icon")).toBeInTheDocument();
      expect(screen.getByText("Enter Realm")).toBeInTheDocument();
    });
  });
