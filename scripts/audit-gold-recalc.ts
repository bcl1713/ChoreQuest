...
async function auditGoldRecalc(userName: string) {
  ...
  quests.forEach((q) => {
    ...
    const date = new Date(q.completed_at || q.updated_at || "")
      .toISOString()
      .slice(0, 10);
    console.log(
      `${date} | ${q.title.padEnd(30).slice(0, 30)} | ${base.toString().padStart(4)} | ${volBonus.toFixed(2)} | ${strBonus.toFixed(2)} | ${payout.toString().padStart(4)}`,
    );
  });
  console.log(
    "--------------------------------------------------------------------",
  ...
  redemptions.forEach((r) => {
    const rewardName = r.reward_name || "Unknown";
    ...
    if (status === "DENIED") {
      ...
      console.log(
        `${date} | ${rewardName.padEnd(30).slice(0, 30)} | ${status.padEnd(8)} | 0 (Refunded)`,
      );
      return;
    }
    ...
    const date = new Date(r.requested_at || "").toISOString().slice(0, 10);
    console.log(
      `${date} | ${rewardName.padEnd(30).slice(0, 30)} | ${status.padEnd(8)} | ${cost}`,
    );
  });
  console.log(
    "--------------------------------------------------------------------",
  ...
  const actualGold = character.gold || 0;
  const diff = actualGold - theoreticalBalance;

  console.log("\n=== SUMMARY ===");
  console.log(`Total Earned:        ${totalEarned}`);
  console.log(`Total Spent:        -${totalSpent}`);
  console.log(`Theoretical Balance: ${theoreticalBalance}`);
  console.log(`Actual Balance:      ${actualGold}`);
  ...
  if (diff !== 0) {
    ...
    console.log(
      '4. "Boss Battle" rewards or other sources not covered by quests.',
    );
  }
}
