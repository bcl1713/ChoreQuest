...
describe("RewardService - rewards", () => {
  ...
  describe("getRewardsForFamily", () => {
    it("fetches all rewards for a family", async () => {
  ...
  describe("createReward", () => {
    ...
    it("throws when creation fails", async () => {
      ...
      ).rejects.toThrow("Failed to create reward: Creation failed");
    });
  });

  describe("updateReward", () => {
    it("updates an existing reward", async () => {
      const updatedReward = { ...mockReward, name: "Updated Screen Time", cost: 150 };
      mockUpdate.mockReturnValue({ eq: mockEq });
      mockEq.mockReturnValue({ select: mockSelect });
      mockSelect.mockReturnValue({ single: mockSingle });
      mockSingle.mockResolvedValue({ data: updatedReward, error: null });

      const result = await service.updateReward(mockRewardId, { name: "Updated Screen Time", cost: 150 });

      expect(mockFrom).toHaveBeenCalledWith("rewards");
      expect(mockUpdate).toHaveBeenCalledWith({ name: "Updated Screen Time", cost: 150 });
    ...
    it("throws when update fails", async () => {
      ...
      await expect(service.updateReward(mockRewardId, { name: "Updated Name" })).rejects.toThrow(
  ...
  describe("deleteReward", () => {
    ...
    it("hard deletes a reward", async () => {
