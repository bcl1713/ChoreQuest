...
describe("QuestCreateModal - template validation", () => {
  ...
  it("shows error when template service fails", async () => {
    ...
    ).mockRejectedValue(new Error("Failed to create quest from template"));
    renderTemplateModal({
      onClose: mockOnClose,
      onQuestCreated: mockOnQuestCreated,
    });
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/failed to create quest from template/i),
      ).toBeInTheDocument();
      expect(mockOnQuestCreated).not.toHaveBeenCalled();
  ...
  it("validates future due date in template mode", async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        questTemplateService.createQuestFromTemplate,
