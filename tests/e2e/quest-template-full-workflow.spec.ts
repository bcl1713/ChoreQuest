...
test.describe('Quest Template Full Workflow Integration', () => {
  test('complete quest template lifecycle with class bonuses and deletion safety', async ({
    ...
    // ============================================================
    await setupUserWithCharacter(page, 'workflow-test', { characterClass: 'MAGE' });
    ...
    await page.fill('[data-testid="template-title-input"]', 'Epic Cleaning Quest');
    await page.fill(
    ...
    await page.selectOption('[data-testid="template-difficulty-select"]', 'HARD');
    ...
    const customTemplate = page.locator('[data-testid^="template-card-"]').filter({
    ...
    await expect(page.locator('[data-testid="template-preview"]')).toContainText(
      'Epic Cleaning Quest'
    );
    await expect(page.locator('[data-testid="template-preview"]')).toContainText('200 XP');
    await expect(page.locator('[data-testid="template-preview"]')).toContainText('100');
    ...
    await expect(page.getByRole('heading', { name: 'Epic Cleaning Quest' }).first()).toBeVisible();

    // ============================================================
    // STEP 4: Guild Master picks up the quest themselves
    // ============================================================
    // Quest should be in "Available Quests" section
    const questCard = page.locator('.fantasy-card').filter({ hasText: 'Epic Cleaning Quest' });
    ...
    const startButton = activeQuestCard.locator('[data-testid="start-quest-button"]');
    await expect(startButton).toBeVisible();
    await startButton.click();
    await page.waitForTimeout(1000);

    // Complete the quest (IN_PROGRESS -> COMPLETED)
    const completeButton = activeQuestCard.locator('[data-testid="complete-quest-button"]');
    ...
    const currentXPText = await page.locator('[data-testid="character-xp"]').textContent();
    const currentGoldText = await page.locator('[data-testid="character-gold"]').textContent();
    const currentXP = parseInt(currentXPText?.match(/\d+/)?.[0] || '0');
    const currentGold = parseInt(currentGoldText?.match(/\d+/)?.[0] || '0');

    // Approve the quest
    const approveButton = activeQuestCard.locator('[data-testid="approve-quest-button"]');
    await expect(approveButton).toBeVisible();
    await approveButton.click();
    await page.waitForTimeout(2000); // Wait for database update and realtime sync
    ...
    await page.reload(); // Manual refresh to ensure we get latest data
    await page.waitForTimeout(1000);
    ...
    const firstAvailableTemplate = await newTemplateSelect.locator('option').nth(1);
    const firstTemplateValue = await firstAvailableTemplate.getAttribute('value');
  ...
  test('family isolation - templates are family-scoped', async ({ browser }) => {
    ...
    await family1Page.fill('[data-testid="template-title-input"]', 'Family 1 Private Quest');
    ...
    await setupUserWithCharacter(family2Page, 'family2-isolation', { characterClass: 'RANGER' });
    ...
    await expect(family2Page.getByTestId('quest-template-manager')).toBeVisible();

    // Verify family 2 CANNOT see family 1's custom template
    await expect(family2Page.getByText('Family 1 Private Quest')).not.toBeVisible();

    // Verify family 2 has their own default templates (from migration)
    const family2Templates = family2Page.locator('[data-testid^="template-card-"]');
    ...
    await family2Page.fill('[data-testid="template-title-input"]', 'Family 2 Private Quest');
    ...
    await expect(family1Page.getByText('Family 2 Private Quest')).not.toBeVisible();
