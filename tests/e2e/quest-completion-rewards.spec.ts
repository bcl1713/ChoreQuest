...
test.describe("Quest Completion Rewards", () => {
  ...
  test("different difficulty multipliers", async ({ page }) => {
    ...
    await createAndCompleteQuest(
      page,
      `Medium Task ${timestamp}`,
      "MEDIUM",
      100,
    );
...
async function createAndCompleteQuest(
  page: Page,
  title: string,
  difficulty: string,
  xp: number,
