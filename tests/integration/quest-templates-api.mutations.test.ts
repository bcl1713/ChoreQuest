...
import { PATCH as PATCH_BY_ID, DELETE } from "@/app/api/quest-templates/[id]/route";
...
describe("Quest Templates API - mutations", () => {
  ...
  describe("PATCH /api/quest-templates/[id]", () => {
    it("updates a template for a Guild Master", async () => {
      ...
      const response = await PATCH_BY_ID(req, { params: Promise.resolve({ id: mockTemplate.id }) });
      const json = await response.json();
