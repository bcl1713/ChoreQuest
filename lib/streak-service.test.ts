...
const createStreakRecord = (
  overrides: Partial<CharacterQuestStreakRow> = {},
): CharacterQuestStreakRow => ({
...
describe("StreakService", () => {
    ...
    it("should increment an existing streak", async () => {
      ...
      const initialStreak = createStreakRecord({
        ...
        last_completed_date: new Date(
          completedDate.getTime() - 86400000,
        ).toISOString(),
      });
      const updatedStreak = {
        ...initialStreak,
      ...
      const streak = await streakService.incrementStreak(
        "char-1",
        "template-1",
        completedDate,
      );
  ...
  describe("validateConsecutiveCompletion", () => {
    ...
    it("should return true for a daily quest completed two days ago (within 2-day window)", () => {
      ...
      // diffInDays = 2.0, and implementation allows <= 2
      expect(result).toBe(true);
    });
