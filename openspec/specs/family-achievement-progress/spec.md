...
Defines the service layer for evaluating and tracking family-scoped
achievement progress. Covers the `FamilyAchievementProgressService`,
family evaluator registry, evaluation modes (sum/all), progress JSONB
shape, upsert strategy, unlock evaluation, integration with the individual
achievement progress service, and the family achievements API route.
